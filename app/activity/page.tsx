...
export default function Activity() {
  ...
  const [leetcodeoverallQuestionData, setLeetcodeoverallQuestionData] =
    useState<any>(null);
  ...
  useEffect(() => {
    ...
    const fetchGitHubRepos = async () => {
      const res = await fetch(
        `https://api.github.com/users/Mananwebdev160408/repos?per_page=3&sort=updated`,
      );
    ...
    const fetchGitHubProfile = async () => {
      const res = await fetch(`https://api.github.com/users/Mananwebdev160408`);
      const data = await res.json();
      setGithubdata(data);
    };
    ...
    const fetchLeetCodeQuestionData = async () => {
      try {
        const res = await fetch(
          `https://alfa-leetcode-api.onrender.com/Manan_160408/solved`,
        );
        const data = await res.json();
    ...
    const fetchGitHubContributions = async () => {
      try {
        ...
        const sum = Object.values(data.total).reduce(
    ...
    const fetchLeetCodeData = async () => {
      try {
        const statsRes = await fetch(
          `https://alfa-leetcode-api.onrender.com/Manan_160408`,
        );
        const stats = await statsRes.json();
        setLeetcodeData(stats);

        const subRes = await fetch(
          `https://alfa-leetcode-api.onrender.com/Manan_160408/acSubmission?limit=4`,
        );
  ...
  return (
    <div className="w-full max-w-[1400px] mx-auto px-6 py-32 relative z-10">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-12 lg:gap-8 items-start">
            ...
            <div className="relative aspect-square w-full max-w-[400px] mx-auto lg:mx-0 border border-white/10 bg-zinc-900/50 p-8 flex items-center justify-center rounded-lg backdrop-blur-sm group">
              ...
              <div className="absolute inset-0 flex flex-col items-center justify-center text-center">
                ...
                <span className="text-xl text-slate-400 font-mono">
                  {leetcodeoverallQuestionData?.length}
                </span>
                <span className="text-xs text-primary mt-2 font-bold tracking-widest uppercase">
                  Solved
                </span>
              </div>
            </div>
          ...
          <div className="lg:col-span-7 flex flex-col gap-12">
            <div className="grid grid-cols-1 md:grid-cols-3 gap-0 border border-white/20 divide-y md:divide-y-0 md:divide-x divide-white/20">
              <DifficultyBar
                label="Easy"
                solved={leetcodeQuestionData?.easySolved || 0}
                total={
                  leetcodeoverallQuestionData?.filter(
              ...
              <DifficultyBar
                label="Medium"
                solved={leetcodeQuestionData?.mediumSolved || 0}
                total={
                  leetcodeoverallQuestionData?.filter(
              ...
              <DifficultyBar
                label="Hard"
                solved={leetcodeQuestionData?.hardSolved || 0}
                total={
                  leetcodeoverallQuestionData?.filter(
...
function DifficultyBar({
  ...
  return (
    <div
      className={`p-6 bg-zinc-900/20 group ${hoverColor} hover:text-black transition-colors duration-300`}
    >
      ...
      <div className="flex justify-between items-end">
        ...
        <span className="text-[10px] px-2 py-1 bg-white/5 border border-white/10 rounded group-hover:border-black/20">
          SOLVED
        </span>
      </div>
...
function RepoCard({ repo }: any) {
  return (
    <a
      href={repo.html_url}
      target="_blank"
      className="group p-6 border border-white/10 bg-zinc-900/20 hover:border-primary/50 transition-colors relative overflow-hidden"
    >
      <div className="absolute top-0 right-0 p-4 opacity-50 group-hover:opacity-100 transition-opacity">
        <span className="material-symbols-outlined transform -rotate-45 text-sm">
          arrow_forward
        </span>
      </div>

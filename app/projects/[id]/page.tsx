        ...
        <aside className="w-full lg:w-64 lg:fixed lg:h-[calc(100vh-80px)] lg:border-r border-white/20 p-8 flex flex-col justify-between bg-black/50 backdrop-blur-sm lg:mt-20">
          <div className="space-y-12 font-mono text-xs tracking-wider">
            <div className="space-y-2">
              <p className="text-[#666]">CLIENT</p>
              <p className="text-white text-sm">INTERNAL LABS</p>
            </div>
            <div className="space-y-2">
            ...
            <div className="space-y-2">
              ...
              <p className="text-white text-sm">{project.category.toUpperCase()}</p>
            </div>
            <div className="space-y-2">
          ...
          <section className="relative px-6 pt-20 pb-10 lg:px-12 lg:pt-32 overflow-hidden">
            <h1 className="text-[14vw] leading-[0.8] font-bold tracking-tighter uppercase break-words mix-blend-difference text-white select-none">
              {project.title.split(" ").map((word, i) => (
                <span key={i} className={`block ${i === 1 ? 'text-outline text-transparent lg:ml-24' : 'ml-[-1vw]'}`}>
                  {word}
                </span>
              ))}
            </h1>
            <div className="absolute top-10 right-10 w-32 h-32 border border-[#333] rounded-full flex items-center justify-center animate-spin duration-[10s]">
              <span className="material-symbols-outlined text-[#333] text-6xl">all_inclusive</span>
            </div>
          </section>
          ...
          <section className="px-6 lg:px-12 mb-20">
            <div className="relative w-full aspect-video border border-[#333] bg-[#111] overflow-hidden group">
              ...
              <div className="absolute bottom-4 right-4 bg-black border border-white px-3 py-1 z-20">
                <span className="font-mono text-xs font-bold text-white tracking-widest">IMG_SEQ_{project.id}.RAW</span>
              </div>
              <div className="absolute top-4 left-4 z-20">
                <span className="material-symbols-outlined text-white/50 text-4xl">crop_free</span>
              </div>
            </div>
          </section>
          ...
          <section className="px-6 lg:px-12 pb-32 grid grid-cols-1 md:grid-cols-12 gap-12 border-t border-[#333] pt-12">
            <div className="md:col-span-7">
              ...
              <p className="font-mono text-sm text-[#888] leading-relaxed max-w-prose">
                By bypassing traditional database structures, we engineered a fluid data lake that mimics synaptic responses. The result is an interface that feels less like a tool and more like an extension of thought.
              </p>
            </div>

            <div className="md:col-span-5 flex flex-col justify-between h-full space-y-12">
              <div>
                <h3 className="font-mono text-[#666] text-xs mb-6 uppercase tracking-widest border-b border-[#333] pb-2">Tech Stack</h3>
                <div className="flex flex-wrap gap-3">
                  {project.tags?.map((tag) => (
                    <span key={tag} className="px-4 py-2 border border-[#444] text-white font-mono text-xs uppercase tracking-wide hover:bg-white hover:text-black hover:border-white transition-all cursor-default">
                      {tag}
                    </span>
                  ))}
              ...
              <div className="pt-8 border-t border-[#333]">
                <a className="group relative block w-full bg-white text-black h-16 flex items-center justify-between px-6 overflow-hidden hover:bg-primary transition-colors duration-300" href="#">
                  <span className="relative z-10 font-bold font-display text-lg tracking-wider">VIEW SOURCE</span>
                  <span className="relative z-10 material-symbols-outlined transform group-hover:translate-x-2 transition-transform">arrow_forward</span>
                </a>
                <div className="mt-2 flex justify-between font-mono text-[10px] text-[#444] uppercase">
                  <span>Repo: Github/{project.title.replace(" ", "_")}</span>
                  <span>Size: 42MB</span>
                </div>
              </div>
            </div>
          ...
          <section className="border-t border-[#333]">
             <div className="grid grid-cols-2 md:grid-cols-4 divide-x divide-[#333] border-b border-[#333]"> 
                 {project.gallery?.map((img, i) => (
                     <div key={i} className="aspect-square relative group overflow-hidden bg-[#0a0a0a]">
                         <Image
                            src={img}
                            alt={`Gallery ${i}`}
                            fill
                         ...
                         />
                         <div className="absolute bottom-2 left-2 text-[10px] font-mono bg-black text-white px-1">SYS_LOG_0{i+1}</div>
                     </div>
                 ))}
                 <div className="aspect-square flex items-center justify-center bg-black hover:bg-[#111] transition-colors cursor-pointer group">
                    <div className="text-center">
                        <span className="material-symbols-outlined text-white text-4xl mb-2 group-hover:scale-110 transition-transform">grid_view</span>

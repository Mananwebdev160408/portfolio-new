...
export default function About() {
  ...
  return (
    <div className="relative min-h-screen flex flex-col font-display text-slate-100 overflow-x-hidden selection:bg-primary selection:text-white bg-background-dark">
      ...
      <main className="flex-grow w-full max-w-[1400px] mx-auto px-6 pt-32 pb-24 relative z-10">
        <Marquee text="FULL STACK • PROBLEM SOLVER • ENGINEER • CREATIVE" />
        <Marquee text="FULL STACK • PROBLEM SOLVER • ENGINEER • CREATIVE" reverse />
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-12 items-start mt-20">
          ...
          <aside className="lg:col-span-3 order-2 lg:order-1 flex flex-col gap-8 sticky top-32">
            <div className="border-l-2 border-primary pl-4">
              <h3 className="text-xs font-bold text-primary tracking-[0.2em] mb-6">HISTORY_LOG</h3>
              <ul className="space-y-6 font-mono text-sm">
                {history.map((item, index) => (
                    <li key={index} className="group">
                        <div className="text-white font-bold group-hover:text-primary transition-colors">{item.year} / {item.role}</div>
                        <div className="text-slate-500 text-xs mt-1">{item.company?item.company:""}</div>
                    </li>
                ))}
              </ul>
            </div>
            <div className="hidden lg:block pt-12">
              <h3 className="text-xs font-bold text-slate-500 tracking-[0.2em] mb-4">LOCATION</h3>
          ...
          <article className="lg:col-span-5 order-3 lg:order-2 flex flex-col gap-10">
            <div className="space-y-8">
              <h1 className="text-5xl md:text-7xl font-bold leading-[0.9] tracking-tight text-white">
                Constructing <span className="text-primary">Logic</span> from Chaos.
              </h1>
              <div className="space-y-6 text-lg md:text-xl font-light leading-relaxed text-slate-300">
                <p>
                  I don&apos;t just write code; I engineer systems. My approach is rooted in the raw fundamentals of computer science, stripped of excess. In a world of bloat, I choose <span className="text-white font-normal border-b border-white/20">precision</span>.
                </p>
                <p>
                  Starting as a self-taught enthusiast breaking hardware in my garage, I&apos;ve evolved into constructing scalable architectures for enterprise-grade applications. The medium changes, but the mission remains: solve the unsolvable.
                </p>
                <p>
                  Whether it&apos;s optimizing a rendering engine or architecting a distributed backend, I find beauty in the brute force of efficient algorithms.
                </p>
              </div>
            </div>
            ...
            <div className="mt-8">
              <div className="relative group">
                ...
                <div className="relative flex flex-col md:flex-row items-start md:items-center justify-between bg-black border border-white/20 p-6 md:p-8">
                  <div>
                    <h3 className="text-xs font-bold text-primary tracking-widest mb-2">CURRENTLY LEARNING</h3>
                    <div className="flex flex-wrap gap-2 text-xl md:text-3xl font-bold text-white tracking-tighter">
                      {learningStack.map((stack, i) => (
                           <span key={stack.name} className="flex gap-2">
                               {stack.name}
                               {i < learningStack.length - 1 && <span className="text-primary">/</span>}
                           </span>
                      ))}
                    </div>
                  </div>
                  <div className="mt-4 md:mt-0">
                    <span className="material-symbols-outlined text-4xl text-primary animate-pulse">data_object</span>
                  </div>
                </div>
              </div>
            </div>
          ...
          <div className="lg:col-span-4 order-1 lg:order-3 grid grid-cols-3 gap-1 lg:h-[600px] h-[300px] w-full">
             {/* Panels would normally be separate images but for now using the same image with object position shifts */}
             {[0, 1, 2].map((i) => (
                 <div key={i} className={`relative h-full overflow-hidden bg-zinc-900 border border-white/5 group ${i === 1 ? 'mt-4 lg:mt-8 mb-0 lg:mb-8' : ''}`}>
                     <div 
